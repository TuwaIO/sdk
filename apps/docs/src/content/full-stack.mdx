...
import { isSessionMatchingTarget, type SiwxClientSession } from '@tuwaio/sdk/siwx';
...
export async function syncTransaction(tx: TransactionUnion, session: SiwxClientSession | SiwxSession | null) {
  if (!session) {
...
export async function getHistory(params: GetHistoryParams, session: SiwxClientSession | SiwxSession | null) {

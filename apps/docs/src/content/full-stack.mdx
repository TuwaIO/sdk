...
import { createSiwxApiHandler } from '@tuwaio/sdk/siwx/server';
...
export async function syncTransaction(tx: TransactionUnion, session: SiwxSession) {
  const isValid = await verifySiwxPayload(session);
...
export async function getHistory(params: any, session: SiwxSession) {
  const isValid = await verifySiwxPayload(session);
  if (!isValid) throw new Error('Invalid signature.');
